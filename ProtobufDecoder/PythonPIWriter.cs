...
namespace ProtobufDecoder
{
	...
	{		
		...
		public PythonPIWriter(Dictionary<string, ObjectDescription> items) : base(items)
		{
		}
		...
		protected void Dump(string path, bool split) {
			...
			foreach (var item in Items) {
				...
				if (c != null && c.ServiceEnum != null && c.ServiceEnum.Items.ContainsKey("CmdId")) {
					if (split)
						w = CreatePythonWriter(path, item.Key.CutAfterPlusAndDot() + ".py");
					
					var cmdId = c.ServiceEnum.Items["CmdId"].Value;
					var channelId = -1;
			...
			foreach (var item in Items) {
				...
				if (c != null) {
					...
					foreach (var type in types) {
						if (type.StartsWith("Google.Protobuf.Collections.MapField")) {
